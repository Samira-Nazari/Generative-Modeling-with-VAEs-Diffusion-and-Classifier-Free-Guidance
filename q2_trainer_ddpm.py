...
class Trainer:
    ...
    def sample(self, n_steps=None, set_seed=False):
        ...
        with torch.no_grad():
            ...
            for t_ in tqdm(range(n_steps)):
                ...
                # TODO: Sample x_t 
                t = torch.full((x.shape[0],), n_steps - 1 - t_, dtype=torch.long, device=self.args.device)
    ...
    def generate_intermediate_samples(self, n_samples=4, img_size=32, steps_to_show=[0,999], n_steps=None, set_seed=False):
        ...
        for step in tqdm(range(1, n_steps+1, 1)):
            ...
            # t = torch.full((x.shape[0],), n_steps - step, dtype=torch.long, device=args.device)
            batch_size = x.shape[0]
            t_value = n_steps - step
            t = torch.full((batch_size,), t_value, dtype=torch.long, device=self.args.device)
            x = self.diffusion.p_sample(x, t)
            ...
            if step in steps_to_show:
                ...
                x_detached = x.detach()
                x_cpu = x_detached.cpu()
                x_numpy = x_cpu.numpy()
                images.append(x_numpy)
