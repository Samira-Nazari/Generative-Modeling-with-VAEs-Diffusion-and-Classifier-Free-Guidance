    ...
    def sample(self, labels=None, cfg_scale=3., n_steps=None, set_seed=False):
        ...
        with torch.no_grad():
            ...
            for t_ in tqdm(range(n_steps)):
                ...
                # Get lambda(t) and lambda(t') (where t' = t-1)
                lambda_t = self.diffusion.get_lambda(t)
                t_next = torch.clamp(t - 1, min=0)
                lambda_t_prim = self.diffusion.get_lambda(t_next)
                ...
                # eps_cond = self.eps_model(z_t, t, labels)  # εθ(z_t, c)
                # eps_uncond = self.eps_model(z_t, t, None)  # εθ(z_t)
                eps_cond = self.eps_model(z_t, labels)  # With labels
                eps_uncond = self.eps_model(z_t, None)  # Without labels

                # Classifier-Free Guidance interpolation
                eps_hat = (1 + cfg_scale) * eps_cond - cfg_scale * eps_uncond
                    
                #TODO: Get x_t then sample z_t from the reverse process according to 4. and 5. in Algo 2.
                # Step 4: Compute x_t (reconstruction of clean image)
                alpha_lambda_t = self.diffusion.alpha_lambda(lambda_t)
                sigma_lambda_t = self.diffusion.sigma_lambda(lambda_t)
                x_t = (z_t - sigma_lambda_t * eps_hat) / alpha_lambda_t

                # Step 5: Sample z_{t+1} from reverse process
                z_t = self.diffusion.p_sample(
                z_lambda_t=z_t,
                lambda_t=lambda_t,
                lambda_t_prim=lambda_t_prim,
                x_t=x_t
                # DO NOT add set_seed here!
                )
